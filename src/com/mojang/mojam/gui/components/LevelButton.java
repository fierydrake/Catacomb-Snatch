...
public class LevelButton extends ClickableComponent {
	...
	private boolean buildMinimap(LevelInformation levelInfo) {
		...
		Level l;
		try {
			l = new GameMode().generateLevel(GameInformation.DEFAULT_DIFFICULTY, levelInfo);
		} catch (IOException e) {
			return false;
		}
	...
	public void tick(LocalGameInput input){
		super.tick(input);
		Point mb = input.getMousePosition();
		int relx = mb.x / 2 - (getX() + (getWidth() - MAXDIM) / 2);
		int rely = mb.y / 2 - (getY() + 4);
